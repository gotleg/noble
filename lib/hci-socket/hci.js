...
var AclPacketQueue = new TinyQueue();
...
Hci.prototype.writeAclDataPkt = function(handle, cid, data) {
    while (this._currentPackets < MAX_HCI_ACL_DATA_PACKETS & AclPacketQueue.length > 0)
    {
        var packet = AclPacketQueue.pop();
        this._currentPackets = this._currentPackets + 1;
        this._socket.write(packet);
        console.log("Sending packets in queue. Queue size :"+AclPacketQueue.length)
    }
    ...
    {
      AclPacketQueue.push(pkt);
      console.log("adding packets to queue to prevent buffer overflow. Queue size :"+AclPacketQueue.length);
    }
  };
