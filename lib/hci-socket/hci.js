...
Hci.prototype.writeAclDataPkt = function(handle, cid, data) {
    while (this._currentPackets < MAX_HCI_ACL_DATA_PACKETS & AclPacketQueue.length > 0)
    {
        ...
        debug("Sending packets in queue. Queue size :"+AclPacketQueue.length)
    }
    ...
    else
    {
      ...
      debug("adding packets to queue to prevent buffer overflow. Queue size :"+AclPacketQueue.length);
    }
  };
