...
var MAX_HCI_ACL_DATA_PACKETS = 0;
...
var Hci = function() {
  ...
  this._state = null;
  this._deviceId = null;
  this._currentPackets = 1;
  this._aclPacketQueue = [];
...
Hci.prototype.setSocketFilter = function() {
  ...
  var eventMask1 = (1 << EVT_DISCONN_COMPLETE) | (1 << EVT_ENCRYPT_CHANGE) | (1 << EVT_CMD_COMPLETE) | (1 << EVT_CMD_STATUS) | (1 << EVT_COMPLETED_PACKETS);
...
Hci.prototype.writeAclDataPkt = function(handle, cid, data) {
    ...
    data.copy(pkt, 9);
    if (this._currentPackets < MAX_HCI_ACL_DATA_PACKETS)
    {
      debug('write acl data pkt - writing: ' + pkt.toString('hex'));
      this._currentPackets = this._currentPackets + 1;
      this._socket.write(pkt);
    }
    else
    {
      this._aclPacketQueue.push(pkt);
      debug("adding packets to queue to prevent buffer overflow. queue size :"+this._aclPacketQueue.length);
    }
  };
...
Hci.prototype.onSocketData = function(data) {
  ...
  if (HCI_EVENT_PKT === eventType) {
    ...
    if (subEventType === EVT_DISCONN_COMPLETE) {
      ...
      packetCount = data.readUInt8(6);
      this._currentPackets = this._currentPackets - packetCount;

      while (this._currentPackets < MAX_HCI_ACL_DATA_PACKETS & this._aclPacketQueue.length > 0)
      {
          var packet = this._aclPacketQueue.shift();
          this._currentPackets = this._currentPackets + 1;
          this._socket.write(packet);
          debug("sending packet in queue. queue size :"+this._aclPacketQueue.length)
      }

    } else if (subEventType === EVT_CMD_STATUS) {
      if (Buffer.byteLength(data) < 7) {
        return;
      }
